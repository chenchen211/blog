...
define('IN_TG', true);
//定义个常量，用来指定本页的内容
define('SCRIPT', 'MyFriends_article');
//引入公共文件
...
<div id="list">
    ...
    if ($_rows['u_state'] == 1) {
        echo 'href="index.php"  onclick="return confirm(\'您已被禁用发帖,请联系管理员\')"';
    } else {
        echo 'href="post.php"';
    } ?> class="post">发表帖子</a>
    <ul class="article">
        ...
        //根据登录名查出朋友
        $sql="SELECT * FROM b_article WHERE a_username in (SELECT f_touser FROM b_friend WHERE f_state=1 and (f_fromuser='{$_SESSION['username']}' or f_fromuser='{$_SESSION['admin']}') )";
        ...
        $_result = _query($sql);
        while ($re_friends = _fetch_array_list($_result)) {
            ?>
            <li <?php if ($re_friends['a_type'] < 16){ ?>class="icon<?php echo $re_friends['a_type'] ?><?
            } else {
                echo 'class="icon1"';
            } ?> ">

                <a href="article.php?id=<?php echo $re_friends['a_id'] ?>"><?php echo _title($re_friends['a_title'], 20) ?></a>
            </li>
        <?php }
...
<div id="user">
    ...
    <form action="index.php" method="post">
        <ul class="article">
            <dd>
                <dl>
                    <input type="text" name="search" value="" placeholder="文章标题或关键词"
                           style="width:120px;height:19px;border:1px dashed #333;background:#fff;">
...
<div id="tag">
    ...
    <dl>
        ...
        while ($rows = _fetch_array_list($result)) {
            ?>
            <dd class="tags"><a
                        href="index.php?tag=<?php echo $rows['tag_name'] ?>&tagid=<?php echo $rows['tag_id'] ?>"><?php echo $rows['tag_name'] ?></a>
            </dd>
        <?php } ?>
