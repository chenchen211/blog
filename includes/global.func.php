...
/**
 * 防止重复发帖
 * @param $_now_time 当前时间
 * @param $_pre_time  最近一次发帖时间
 * @param $_second  间隔
 */
...
function _pagings($_type)
{
    global $_page, $_pageabsolute, $_num, $_id;
    if ($_type == 1) {
        echo '<ul class="am-pagination am-fr">';
        for ($i = 0; $i < $_pageabsolute; $i++) {
            if ($_page == ($i + 1)) {
                echo '<li class="am-active"><a href="' . SCRIPT . '.php?' . $_id . 'page=' . ($i + 1) . '" class="selected">' . ($i + 1) . '</a></li>';
...
{
    if (strtoupper($_first_code) != strtoupper($_end_code)) {//转换成大写
        _alert_back('验证码不正确!');
    }
}
